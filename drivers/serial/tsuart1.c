...
static void tsuart_mdm_timer(unsigned long data) {
  ...
  while (root) {
	...
	//wake_up_interruptible(&next->u.state->delta_msr_wait);
      }
      next = next->next;
    }
    root = root->nextd;
...
static void tsuart_cts_timer(unsigned long data) {
  ...
  tsuart_checkHandleTx(port+1);
  tsuart_checkHandleTx(port+2);
  */
}
...
static unsigned int tsuart_get_mctrl(struct uart_port *port) {
  ...
  if (bit_CTS(tsuartGetSTAT(TSPORT(port)))) {
    result |= TIOCM_CTS;
  }
  if (TSPORT(port)->debugFlags & DEBUG_INTERFACE) {
...
static void tsuart_set_mctrl(struct uart_port *port, unsigned int mctrl) {
  ...
  if (TSPORT(port)->debugFlags & DEBUG_INTERFACE) {
    printk("tsuart_set_mctrl(%d,RTS=%d)\n",port->line,((mctrl&TIOCM_RTS) != 0) ? 1:0);
  }
  tsuartPutSTAT(p,assign_bit_RTS(tsuartGetSTAT(p),(mctrl&TIOCM_RTS)!=0));
  if (p->mdmctrl) {
...
static int tsuart_request_port(struct uart_port *port) {
  ...
  if (port->iotype == SERIAL_IO_MEM) {
    if (port->membase == 0) {
      port->membase = (void *) __ioremap(port->mapbase & 0xFFFFF000, 4096,0)
	+ (port->mapbase & 0xFFF);
