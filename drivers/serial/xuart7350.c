/*
 *  linux/drivers/serial/xuart7350.c
 *
 *  Linux 2.6 Driver for TS-XUART on TS-7350/TS-7370
 *
 ...
 *
 * 2008-09-10:mos:created initial version from xuart.c
 */
...
static void xuart_try_handleTx(struct xuart_port *port) {
  ...
  */
  ...
  while (i < 512) { // handle maximum of 512 characters at once
    ...
    if (i < 23) debug[i] = ch;
    if ((q=xu_writec(port->xu,port->u.line,(unsigned char *)&ch,1)) < 1) {
      break;
    }
    if (port->debugFlags & DEBUG_DATA) {
      unsigned int ttyinf = port->u.state ? (port->u.state->port.tty ? 3:1) : 0;
...
static void xuart_stop_tx(struct uart_port *port) {
  ...
  while ((n+=xu_draintx(p->xu,p->u.line)) && i < 1000000) i++;

  printk("xuart_stop_tx waited %d (%d)\n",i,n);
...
static int xuart_startup(struct uart_port *port)
{
  ...
  /* Uncomment to force port to always be low latency
  if (port->info->tty) {
    printk("XUART: low latency was %d, now SET\n",port->info->tty->low_latency);
...
static void xuart_shutdown(struct uart_port *port)
{
  ...
  j = xu_draintx(p->xu,p->u.line);
  while (xu_draintx(p->xu,p->u.line) && i < 1000000) i++;
  if (i == 1000000) printk("hit limit waiting for Tx to drain in shutdown\n");
  else printk("i=%d, j=%d\n",i,j);
...
static int xuart_verify_port(struct uart_port *port, struct serial_struct *ser)
{
  ...
  if (XPORT(port)->debugFlags & DEBUG_INTERFACE) {
    printk("xuart_verify_port(%d,%d)=%d baud=%d\n",port->line,ser->irq,ret,
	   ser->baud_base);
  }
  return ret;
}
...
{
  if (XPORT(port)->debugFlags & DEBUG_INTERFACE) {
    printk("xuart_ioctls: port %d, cmd=%X, arg=%X\n",port->line,cmd,(int)arg);
  }
  return -ENOIOCTLCMD;
}

static int txrxstatus(struct xuart_port *port) {
  int ret = 0xFFFF;

  while (port) {
    if (!port->u.state || uart_circ_empty(&port->u.state->xmit)) {
      ret &= ~(1 << (8+port->u.line));
    }
    port = port->next;
  }
...
static irqreturn_t xuart_interrupt(int irq, void *dev_id) {
 ...
 while ((status = txrxstatus(port)) & (ret=xu_irq(port->xu))) {
    ret = ret & status;
    n++;
    while (port) {
      if ((ret >> port->u.line) & 1) {
	xuart_try_handleRx(port);
      }
      if ((ret >> (8+port->u.line)) & 1) {
	xuart_try_handleTx(port);
      }
      port = port->next;
    }
    port = (struct xuart_port *)dev_id;
  }

  if (port->debugFlags & DEBUG_INTERRUPT) {  
    printk("xuart_interrupt ended (%d)\n",n);
  }
...
int xuart_register_port(struct uart_driver *driver,
  ...
  if ((ret = uart_add_one_port(driver,p)) == 0) {
    ...
    if (irq_requested) {
      ...
      ret = request_irq(port->u.irq, xuart_interrupt, 
			IRQF_SHARED, 
			"xuart_uarts", p);
    }
    if (ret != 0) {
      ...
      if (!irq_requested && port->debugFlags & DEBUG_INTERRUPT) {
...
/*
UART#0 - RS232 on TX/RX on COM2 header
UART#1 - RS232 on RTS/CTS on COM1 header
UART#2 - RS232 on DTR/DCD on COM1 header
UART#3 - RS485 on DSR/DCD on COM2 header 
UART#4 - RS485 on DTR/RI on COM2 header (or full duplex RX of UART#3)
...
static int __init xuart_init(void)
{
  ...
  xu->xu_memstart = (int)__arm_ioremap(0x60000000, 8192,0);
  ...
  if (n < 1) {
    ...
    printk("xuartcore reports %d XUART ports\n",n);
  }

  for (i=0;i<n;i++) {
    ts = kmalloc(sizeof(struct xuart_port),GFP_KERNEL);
    if (ts0) {
      ts0->next = ts;
    }
    if (!ts) {
