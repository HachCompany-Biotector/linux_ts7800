 ...
 * 2013-01-07:mlf:updated for TS-7800 with module arguments for others
*/
...
#define TS7800_ISA8_BUS 	0xEE000000
#define TS7800_ISA16_BUS 	0xEF000000
#define IRQ_MARVELL_ISA 	64+5
...
/*
static int tsrf2_ack_intr(ide_hwif_t *hwif) 
{
	// Do something to ack the interrupt

	printk("I should ack the interrupt here\n");
	if(0)
		return 0;
	else
		return 1;
...
irqreturn_t tsrf2_irq_handler(int irq, void *dev_id)
{
	printk("TS-RF2-CF: IRQ %d handled\n", irq);
...
static int tsrf2_register(unsigned long addr8, unsigned long addr16, unsigned long offset, int irq)
{
	...
	host->irq_handler = tsrf2_irq_handler;
...
{
	// Assumes the TS-7800, but module arguments will be used instead if set
	unsigned long addr8 = TS7800_ISA8_BUS;
	unsigned long addr16 = TS7800_ISA16_BUS;
...
static int __exit tsrf2_exit(void)
{
	return 0;
}
