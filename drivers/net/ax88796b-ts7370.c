...
#define TS7XXX_PC104IO8		0xee000000
#define TS7XXX_PC104IO16	0xef000000
#define BASE_IRQ 64
#endif
...
#define inb(x) readb(x)
#define outb(x,y) writeb(x,y)
#define inw(x) readw(x)
#define outw(x,y) writew(x,y)
#define inl(x) readl(x)
#define outl(x,y) writel(x,y)


#if defined(CONFIG_MACH_TS7350)
#define DEFMEM		0x800
#define DEFIRQ		81
#define DEFIOBASE	0x600ff000
#else
#define DEFMEM          0x0
#define DEFIRQ          0
...
int __init ax_init_module(void)
{
...
#ifdef MODULE
	struct net_device *dev = alloc_netdev(sizeof(struct net_device), "eth%d", fool_init);
#else
	struct net_device *dev = alloc_netdev(sizeof(struct net_device), "eth1", fool_init);
	...
	if (dev->base_addr != 0) {
	  PRINTK(WARNING_MSG, "AX88796B: No AX88796B card found at memory = %#x, error = %d\n", dev->base_addr, ret);
	}
	else {
...
{
	int i;
	unsigned char SA_prom[32], frame_size;
	int wordlength = 0;
	...
	if(dev->base_addr == 0 ) {
		if ( (inb(TS7XXX_PC104IO8|0x100)&0xF) == 0x5 && 
		  (inb(TS7XXX_PC104IO8|0x104)&0xF) == 0x1 ) {
	          dev->base_addr = 0x200;
		  if ((inb(TS7XXX_PC104IO8|0x108)&0xE) == 0x8) dev->irq = 7;
		  else if ((inb(TS7XXX_PC104IO8|0x108)&0xE) == 0x4) dev->irq = 6;
		  else if ((inb(TS7XXX_PC104IO8|0x108)&0xE) == 0x2) dev->irq = 5;		  
		  
		} else if ( (inb(TS7XXX_PC104IO8|0x110)&0xF) == 0x5 && 
		  (inb(TS7XXX_PC104IO8|0x114)&0xF) == 0x1 ) {
	          dev->base_addr = 0x240;
		  if ((inb(TS7XXX_PC104IO8|0x118)&0xE) == 0x8) dev->irq = 7;
		  else if ((inb(TS7XXX_PC104IO8|0x118)&0xE) == 0x4) dev->irq = 6;
		  else if ((inb(TS7XXX_PC104IO8|0x118)&0xE) == 0x2) dev->irq = 5;		  

		} else if ( (inb(TS7XXX_PC104IO8|0x120)&0xF) == 0x5 && 
		  (inb(TS7XXX_PC104IO8|0x124)&0xF) == 0x1 ) {
	          dev->base_addr = 0x300;
		  if ((inb(TS7XXX_PC104IO8|0x128)&0xE) == 0x8) dev->irq = 7;
		  else if ((inb(TS7XXX_PC104IO8|0x128)&0xE) == 0x4) dev->irq = 6;
		  else if ((inb(TS7XXX_PC104IO8|0x128)&0xE) == 0x2) dev->irq = 5;		  
		  
		} else if ( (inb(TS7XXX_PC104IO8|0x130)&0xF) == 0x5 && 
		  (inb(TS7XXX_PC104IO8|0x134)&0xF) == 0x1 ) {
	          dev->base_addr = 0x340;
		  if ((inb(TS7XXX_PC104IO8|0x138)&0xE) == 0x8) dev->irq = 7;
		  else if ((inb(TS7XXX_PC104IO8|0x138)&0xE) == 0x4) dev->irq = 6;
		  else if ((inb(TS7XXX_PC104IO8|0x138)&0xE) == 0x2) dev->irq = 5;		  		  
		}
	}
#else
#endif
        ...
        if (iobase != 0) {
          ...
          io8base = TS7XXX_PC104IO8;
          io16base = TS7XXX_PC104IO16;
        }
	...
	*/
        ...
        address = (void *)ioremap(io8base + base_addr, NE_IO_EXTENT);
 	address16 = (void *)ioremap(io16base + base_addr, NE_IO_EXTENT);	
	
	if (!address) {
		PRINTK(ERROR_MSG, "AX88796B: Unable to remap memory 0x%x\n", address);
		return -EBUSY;
	}

        //detect if Asix frame size reg is writeable/readable
        frame_size = inb(address+0x18);
        outb(0xaa, address+0x18);
        inb(address);   //clear bus memory

        if(inb(address+0x18) != 0xaa) {
	...
	{
		int regd;
		outb(E8390_NODMA+E8390_PAGE1+E8390_STOP, address + E8390_CMD);
		regd = inb(address + EN0_COUNTER0);
		outb(0xff, address + EN0_COUNTER0);
		outb(E8390_NODMA+E8390_PAGE0, address + E8390_CMD);
		inb(address + EN0_COUNTER0); // Clear the counter by reading.
		//inb(address + EN0_COUNTER0);
		udelay(100);
		if (inb(address + EN0_COUNTER0) != 0) {
			outb(reg0, address);
			outb(regd, address + EN0_COUNTER0);	// Restore the old values. 
			ret = -ENODEV;
			goto err_out;
		}
	}
	...
	/* Reset card. Who knows what dain-bramaged state it was left in. */
	{
		...
		outb(inb(address + EN0_RESET),(address + EN0_RESET));
		while ((inb(address + EN0_ISR) & ENISR_RESET) == 0)  {
		...
		outb(0xff,(address + EN0_ISR));		/* Ack all intr. */
	}
	...
	/* Read the 16 bytes of station address PROM. */
	{
		...
		for (i = 0; i < sizeof(program_seq)/sizeof(program_seq[0]); i++)
			outb(program_seq[i].value, address + program_seq[i].offset);
	}

	for(i = 0; i < 32; i+=2) {
		SA_prom[i] = inb(address + EN0_DATAPORT);
		SA_prom[i+1] = inb(address + EN0_DATAPORT);
        ...
        /* 
	{
        ...
        */
		...
		adaux = (void *)ioremap(0x80010050, 0xFF);
                SA_prom[0] = inb(adaux+0x0);
                SA_prom[1] = inb(adaux+0x1);
                SA_prom[2] = inb(adaux+0x2);
                SA_prom[3] = inb(adaux+0x3);
                SA_prom[4] = inb(adaux+0x4);
                SA_prom[5] = inb(adaux+0x5) + 0x1;
		iounmap(adaux);
        }
#endif
...
//EWD: I believe the code below has a bug 20% of the time if this probe fails
//the kernel will crash.
err_out:
        if (address) iounmap(address);
	if (address16) iounmap(address16);
	if (dev->priv != NULL)
          kfree(dev->priv);
	dev->priv = NULL;
	PRINTK(INIT_MSG, "AX88796B: probe error...\n");
...
static void ax_reset(struct net_device *dev)
{
	...
	/* DON'T change these to inb/outb or reset will fail on clones. */
	outb(inb(ax_base + EN0_RESET), ax_base + EN0_RESET);

	ax_local->dmaing = 0;

	/* This check _should_not_ be necessary, omit eventually. */
	while ((inb(ax_base+EN0_ISR) & ENISR_RESET) == 0)
		if (jiffies - reset_start_time > 2*HZ/100) {
			PRINTK(ERROR_MSG, "%s: ax_reset() did not complete.\n", dev->name);
			break;
		}
	outb(ENISR_RESET, ax_base + EN0_ISR);	/* Ack intr. */
}
...
static void ax_get_hdr(struct net_device *dev, struct ax_pkt_hdr *hdr, int ring_page)
{
	...
	outb(E8390_NODMA+E8390_PAGE0+E8390_START, ax_base+ E8390_CMD);
	outb(sizeof(struct ax_pkt_hdr), ax_base + EN0_RCNTLO);
	outb(0, ax_base + EN0_RCNTHI);
	outb(0, ax_base + EN0_RSARLO);		/* On page boundary */
	outb(ring_page, ax_base + EN0_RSARHI);
	outb(E8390_RREAD+E8390_START, ax_base + E8390_CMD);

	while (( inb(ax_base+EN0_SR) & 0x20) ==0);

	for(tmp=0; tmp < (sizeof(struct ax_pkt_hdr)>>1); tmp++) {
      *((u16 *)hdr + tmp)= inw( ax_local->membase16 + EN0_DATAPORT );	
	}

	outb(ENISR_RDC, ax_base + EN0_ISR);	/* Ack intr. */
	ax_local->dmaing = 0;
...
static void ax_block_input(struct net_device *dev, int count, struct sk_buff *skb, int ring_offset)
{
	...
	ax_local->dmaing |= 0x01;
	outb(E8390_NODMA+E8390_PAGE0+E8390_START, ax_base+ E8390_CMD);
	outb(count & 0xff, ax_base + EN0_RCNTLO);
	outb(count >> 8, ax_base + EN0_RCNTHI);
	outb(ring_offset & 0xff, ax_base + EN0_RSARLO);
	outb(ring_offset >> 8, ax_base + EN0_RSARHI);
	outb(E8390_RREAD+E8390_START, ax_base + E8390_CMD);
...
#else
	{
		...
		if(ax_local->vlgrp) {
			...
			if (count & 0x01)
				vlan_buf[count-1] = inb(ax_base + EN0_DATA_ADDR);
			memcpy((void *)(skb->data), vlan_buf, 12);
			memcpy((void *)(skb->data+12), vlan_buf, (count-4-12));
		}
		else
		...
		{
			for(i=0; i < tmp; i++) {
				*((u16 *)buf + i) = inw( ax_local->membase16 + EN0_DATA_ADDR);
			}
			if (count & 0x01)
				buf[count-1] = inb(ax_local->membase16 + EN0_DATA_ADDR);
		}
	}
#endif
...
static void ax_block_output(struct net_device *dev, int count, const unsigned char *buf, const int start_page)
{
	...
	outb(E8390_PAGE0+E8390_START+E8390_NODMA, ax_base + E8390_CMD);
	outb(ENISR_RDC, ax_base + EN0_ISR);
	/* Now the normal output. */
	outb(count & 0xff, ax_base + EN0_RCNTLO);
	outb(count >> 8,   ax_base + EN0_RCNTHI);
	outb(0x00, ax_base + EN0_RSARLO);
	outb(start_page, ax_base + EN0_RSARHI);
	outb(E8390_RWRITE+E8390_START, ax_base + E8390_CMD);
...
#else
	{
		...
		for (i=0; i<count; i+=2)
		{
			...
			outw(data, ax_local->membase16 + EN0_DATAPORT);
		}
	}
#endif
	dma_start = jiffies;
	while ((inb(ax_base + EN0_ISR) & 0x40) == 0) {
...
static int ax_open(struct net_device *dev)
{
	...
	ret = request_irq(dev->irq, &ax_interrupt, IRQF_SHARED/*SA_INTERRUPT*/, dev->name, dev);
...
static int ax_start_xmit(struct sk_buff *skb, struct net_device *dev)
{
	...
	if (vlan_tx_tag_present(skb)) {
		...
		outb((u8)(vlan_tag&0xff), ax_base + EN0_VID0);
		outb((u8)((vlan_tag&0xf00)>>8), ax_base + EN0_VID1);
		ax_local->vid = vlan_tag;
	}
#endif
	spin_lock_irqsave(&ax_local->page_lock, flags);
		outb(0x00, ax_base + EN0_IMR);
	spin_unlock_irqrestore(&ax_local->page_lock, flags);
	...
	ctepr = inb(ax_base + EN0_CTEPR) & 0x7f;
	...
	if(free_pages < need_pages) {
		...
		spin_lock_irqsave(&ax_local->page_lock, flags);
		outb(ENISR_ALL, ax_base + EN0_IMR);
		spin_unlock_irqrestore(&ax_local->page_lock, flags);
		return 1;
	}
#if defined(CONFIG_VLAN_8021Q) || defined(CONFIG_VLAN_8021Q_MODULE)
...
static void ax_tx_intr(struct net_device *dev)
{
    struct ax_device *ax_local = (struct ax_device *) dev->priv;
	void *ax_base = ax_local->membase;
	int status = inb(ax_base + EN0_TSR);
...
static irqreturn_t ax_interrupt(int irq, void *dev_id, struct pt_regs * regs)
{
	...
	outb(E8390_NODMA+E8390_PAGE0, ax_base + E8390_CMD);
	...
	while (1)
	{
                interrupts = inb(ax_base + EN0_ISR);
		if(interrupts == 0) 
			break;
                
		
		outb(interrupts, ax_base + EN0_ISR); /* Ack the interrupts */
		...
		{   
			ax_local->stat.rx_frame_errors += inb(ax_base + EN0_COUNTER0);
			ax_local->stat.rx_crc_errors   += inb(ax_base + EN0_COUNTER1);
			ax_local->stat.rx_missed_errors+= inb(ax_base + EN0_COUNTER2); 
			outb(ENISR_COUNTERS, ax_base + EN0_ISR); /* Ack intr. */
		}
...
static void ax_tx_err(struct net_device *dev)
{
	...
	unsigned char txsr = inb(ax_base+EN0_TSR);
...
static void ax_receive(struct net_device *dev)
{
	...
	while(1)
	{
		...
		outb(E8390_NODMA+E8390_PAGE1, ax_base + E8390_CMD);
		rxing_page = inb(ax_base + EN1_CURPAG);
		outb(E8390_NODMA+E8390_PAGE0, ax_base + E8390_CMD);

		
		/* Remove one frame from the ring.  Boundary is always a page behind. */
		this_frame = inb(ax_base + EN0_BOUNDARY) + 1;
		...
		if (rx_frame.next != next_frame
			...
			outb(ax_local->current_page-1, ax_base+EN0_BOUNDARY);
		...
		outb(next_frame-1, ax_base+EN0_BOUNDARY);
	}
...
static void ax_rx_overrun(struct net_device *dev)
{
	 ...
	 * stop command.
	 */
	was_txing = inb(ax_base+E8390_CMD) & E8390_TRANS;
	outb(E8390_NODMA+E8390_PAGE0+E8390_STOP, ax_base+E8390_CMD);
	...
	outb(0x00, ax_base+EN0_RCNTLO);
	outb(0x00, ax_base+EN0_RCNTHI);
	...
	if (was_txing)
	{ 
		unsigned char tx_completed = inb(ax_base+EN0_ISR) & (ENISR_TX+ENISR_TX_ERR);
		if (!tx_completed)
			must_resend = 1;
	}
	 ...
	 */
	outb(E8390_TXOFF, ax_base + EN0_TXCR);
	outb(E8390_NODMA + E8390_PAGE0 + E8390_START, ax_base + E8390_CMD);
	 ...
	 */
	outb(E8390_TXCONFIG, ax_base + EN0_TXCR); 
	if (must_resend)
    	outb(E8390_NODMA + E8390_PAGE0 + E8390_START + E8390_TRANS, ax_base + E8390_CMD);
...
static void do_set_multicast_list(struct net_device *dev)
{
	...
	if (netif_running(dev))
		outb(E8390_RXCONFIG, ax_base + EN0_RXCR);
	outb(E8390_NODMA + E8390_PAGE1, ax_base + E8390_CMD);
	for(i = 0; i < 8; i++) 
	{
		outb(ax_local->mcfilter[i], ax_base + EN1_MULT_SHIFT(i));
	}
	outb(E8390_NODMA + E8390_PAGE0, ax_base + E8390_CMD);

  	if(dev->flags&IFF_PROMISC)
  		outb(E8390_RXCONFIG | 0x18, ax_base + EN0_RXCR);
	else if(dev->flags&IFF_ALLMULTI || dev->mc_list) 
  		outb(E8390_RXCONFIG | 0x08, ax_base + EN0_RXCR);
...
void ax88796_PHY_init(struct net_device *dev)
{
	...
	outb(ENFLOW_ENABLE, ax_base+EN0_FLOW);
	...
	/* Enable AX88796B TQC */
	tmp_data = inb(ax_base+EN0_MCR);
	outb( tmp_data | ENTQC_ENABLE, ax_base+EN0_MCR);

	/* Enable AX88796B Transmit Buffer Ring */
	outb(E8390_NODMA+E8390_PAGE3+E8390_STOP, ax_base+E8390_CMD);
	outb(ENTBR_ENABLE, ax_base+EN3_TBR);
	outb(E8390_NODMA+E8390_PAGE0+E8390_STOP, ax_base+E8390_CMD);
	...
	ax_local->media_curr = inb(ax_base + EN0_SR);
...
static void ax_init(struct net_device *dev, int startp)
{
	...
	outb(E8390_NODMA+E8390_PAGE0+E8390_STOP, ax_base+E8390_CMD); /* 0x21 */
	outb(0x49, ax_base + EN0_DCFG);	/* 0x48 or 0x49 */
	/* Clear the remote byte count registers. */
	outb(0x00,  ax_base + EN0_RCNTLO);
	outb(0x00,  ax_base + EN0_RCNTHI);
	/* Set to monitor and loopback mode -- this is vital!. */
	outb(E8390_RXOFF, ax_base + EN0_RXCR); /* 0x20 */
	outb(E8390_TXOFF, ax_base + EN0_TXCR); /* 0x02 */
	/* Set the transmit page and receive ring. */
	outb(NESM_START_PG, ax_base + EN0_TPSR);
	outb(NESM_RX_START_PG, ax_base + EN0_STARTPG);
	outb(NESM_RX_START_PG, ax_base + EN0_BOUNDARY);

	ax_local->current_page = NESM_RX_START_PG + 1;		/* assert boundary+1 */
	outb(NESM_STOP_PG, ax_base + EN0_STOPPG);
	...
	outb(0xFF, ax_base + EN0_ISR);
	outb(0x00,  ax_base + EN0_IMR);

	/* Copy the station address into the DS8390 registers. */

	outb(E8390_NODMA + E8390_PAGE1 + E8390_STOP, ax_base+E8390_CMD); /* 0x61 */
	outb(NESM_START_PG + TX_PAGES + 1, ax_base + EN1_CURPAG);
	for(i = 0; i < 6; i++) 
	{
		outb(dev->dev_addr[i], ax_base + EN1_PHYS_SHIFT(i));
		if(inb(ax_base + EN1_PHYS_SHIFT(i))!=dev->dev_addr[i])
	...
	if (startp) 
	{
		...
		outb(ENISR_ALL,  ax_base + EN0_IMR);
		outb(E8390_NODMA+E8390_PAGE0+E8390_START, ax_base+E8390_CMD);
		outb(E8390_TXCONFIG, ax_base + EN0_TXCR); /* xmit on. */
		/* 3c503 TechMan says rxconfig only after the NIC is started. */
		outb(E8390_RXCONFIG, ax_base + EN0_RXCR); /* rx on,  */
...
static void ax_trigger_send(struct net_device *dev, unsigned int length, int start_page)
{
	...
	void *ax_base = ax_local->membase;
	outb(E8390_NODMA+E8390_PAGE0, ax_base+E8390_CMD);
	outb(length & 0xff, ax_base + EN0_TCNTLO);
	outb(length >> 8, ax_base + EN0_TCNTHI);
	outb(start_page, ax_base + EN0_TPSR);
	outb((E8390_NODMA|E8390_TRANS|E8390_START), ax_base+E8390_CMD);
}
...
static void ax_vlan_rx_register(struct net_device *dev, struct vlan_group *grp)
{
	...
	outb((inb(ax_base+EN0_MCR) | ENVLAN_ENABLE), ax_base+EN0_MCR);
	ax_local->vlgrp = grp;
}
...
static void mdio_sync(struct net_device *dev)
{
    ...
    for (bits = 0; bits < 32; bits++) {
		outb(MDIO_DATA_WRITE1, ax_base + AX88796_MII_EEPROM);
		outb(MDIO_DATA_WRITE1 | MDIO_SHIFT_CLK, ax_base + AX88796_MII_EEPROM);
    }
}
...
static void mdio_clear(struct net_device *dev)
{
    ...
    for (bits = 0; bits < 16; bits++) {
		outb(MDIO_DATA_WRITE0, ax_base + AX88796_MII_EEPROM);
		outb(MDIO_DATA_WRITE0 | MDIO_SHIFT_CLK, ax_base + AX88796_MII_EEPROM);
    }
}
...
static int mdio_read(struct net_device *dev, int phy_id, int loc)
{
    ...
    for (i = 14; i >= 0; i--) {
		...
		outb(dat, ax_base + AX88796_MII_EEPROM);
		outb(dat | MDIO_SHIFT_CLK, ax_base + AX88796_MII_EEPROM);
    }
    for (i = 19; i > 0; i--) {
		outb(MDIO_ENB_IN, ax_base + AX88796_MII_EEPROM);
		retval = (retval << 1) | ((inb(ax_base + AX88796_MII_EEPROM) & MDIO_DATA_READ) != 0);
		outb(MDIO_ENB_IN | MDIO_SHIFT_CLK, ax_base + AX88796_MII_EEPROM);
    }
